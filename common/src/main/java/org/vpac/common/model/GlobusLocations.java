...
/**
 * This class is a utility class to return all globus-relevant files.
 * The files are: the [user]/.globus directory, private key, the certificate_request and the certificate.
 * I know this class is pretty useless because I could use CoGProperties.getDefault for all of the fields.
 * Don't know why I did not do that in the first place.
 * Maybe I refactor grix to use the cogproperties  directly when I have time.
 * 
...
public class GlobusLocations {
	...
	static final Logger myLogger = Logger.getLogger(GlobusLocations.class.getName());
	
	static ResourceBundle messages = ResourceBundle.getBundle("ModelMessagesBundle", java.util.Locale.getDefault());
	...
	private GlobusLocations(){
		...
		if ( certDirName == null || "".equals(certDirName) ) {
			...
			this.renewUserCert = new File(cog.getUserCertFile()+RENEW_EXTENSION);
			this.userKey = new File(cog.getUserKeyFile());
			this.renewUserKey = new File(cog.getUserKeyFile()+RENEW_EXTENSION);
			this.globusDirectory = new File( System.getProperty( "user.home" ), GLOBUS_DIR_NAME);
		} else {
			myLogger.debug("Found x509_cert_dir property: "+certDirName+". Using it for cert location and grix properties file.");
			this.globusDirectory = new File ( certDirName );
			if ( ! this.globusDirectory.exists() ) {
				this.globusDirectory.mkdirs();
			}
					
			if ( ! this.globusDirectory.exists() || ! this.globusDirectory.canWrite() ) {
				myLogger.warn("Can't write to certificate directory "+this.globusDirectory.toString()+". Using default location in $HOME/.globus.");
				this.globusDirectory = new File( System.getProperty( "user.home" ), GLOBUS_DIR_NAME);
			}
			this.userCert = new File(this.globusDirectory, "usercert.pem");
			this.renewUserCert = new File(this.userCert.toString()+RENEW_EXTENSION);
			this.userKey = new File(this.globusDirectory, "userkey.pem");
			this.renewUserKey = new File(this.userKey.toString()+RENEW_EXTENSION);
		}
		this.userCertRequest = new File( this.globusDirectory, CERT_REQUEST_FILENAME);
		this.renewUserCertRequest = new File (this.globusDirectory, CERT_REQUEST_FILENAME+RENEW_EXTENSION);
		this.userCertPKCS12 = new File( this.globusDirectory, CERT_PKCS12_FILENAME);
		String[] dirs = null;
		try {
//			dirs = CoGProperties.getDefault().getCaCertLocations().split(",");
//			this.certificatesDirectories = new File[dirs.length];
//			for ( int i = 0; i<dirs.length; i++ ){
//				this.certificatesDirectories[i] = new File(dirs[i]);
//			}
			//File dir = new File(CoGProperties.getDefault().getCaCertLocations().split(",")[0]).getParentFile();
	...
	public File createGlobusDirectory() throws IOException {
		if (!globusDirectoryExists()){
			if (!globusDirectory.mkdirs()) throw new IOException(messages.getString("error.cannotCreateDirectory")+" "+globusDirectory.toString());
		}
