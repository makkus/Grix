 ...
 *
 */
public class GridProxyStatusPanel extends JPanel implements GridProxyListener {
...
//
	...
	private ImageIcon yellowLight = null;  //  @jve:decl-index=0:
	...
	private String tooltipStatus = null;
		...
		updatePanelAction = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ( !warning && LocalProxy.getDefaultProxy().getTimeLeft() < expiredProxyWarning ){
					...
					blinkTimer.start();
					GridProxyStatusPanel.this.setToolTipText(getMessages().getString("status.proxyAboutToExpire"));
					warning = true;
				}
			}
		};
	...
	private void initialize() {
		...
		passphraseFieldConstraints.gridy = 0;
		passphraseFieldConstraints.weighty = 0.0;
		passphraseFieldConstraints.weightx = 1.0;
		passphraseFieldConstraints.insets = new Insets(0, 0, 5, 0);
		passphraseFieldConstraints.gridx = 3;
		GridBagConstraints timeLeftTextFieldConstraints = new GridBagConstraints();
		timeLeftTextFieldConstraints.fill = GridBagConstraints.HORIZONTAL;
		timeLeftTextFieldConstraints.gridy = 0;
		timeLeftTextFieldConstraints.weighty = 0.0;
		timeLeftTextFieldConstraints.weightx = 1.0;
		timeLeftTextFieldConstraints.insets = new Insets(0, 0, 5, 0);
		timeLeftTextFieldConstraints.gridx = 1;
		this.setSize(753, 57);
		this.setLayout(new GridBagLayout());
		this.setPreferredSize(new Dimension(412, 294));
		this.setBackground(new Color(245, 245, 245));
		this.setBorder(BorderFactory.createTitledBorder(null, "Grid proxy", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		this.setName("panel");
	...
	public void gridProxyStatusChanged(GridProxyEvent e) {
		...
		if (GridProxy.INITIALIZED == e.getStatus()) {
			...
			blinkTimer.stop();
			updatePanel.start();
			this.setToolTipText(getMessages().getString("status.initialized"));
		}
			...
			updatePanel.stop();
			blinkTimer.start();
			this.setToolTipText(getMessages().getString("status.initializing"));
		}
			...
			updatePanel.stop();
			blinkTimer.start();
			this.setToolTipText(getMessages().getString("status.missingPrerequisites"));
		}
			...
			updatePanel.stop();
			blinkTimer.stop();
			this.setToolTipText(getMessages().getString("status.notInitialized"));
		}
	...
	private JTextField getTimeLeftTextField() {
		if (timeLeftTextField == null) {
			...
			timeLeftTextField.setPreferredSize(new Dimension(24, 120));
