...
public class VomsMyProxy {
	...
	static final Logger myLogger = Logger.getLogger(VomsMyProxy.class.getName());
	...
	InputStream inCert;
		...
		inCert = new FileInputStream(GlobusLocations.defaultLocations().getUserCert());
		InputStream inKey = new FileInputStream(GlobusLocations.defaultLocations().getUserKey());
...
//	 Extension 1
	//DERSequence seqac = new DERSequence(vomsProxyCredential.getAttributeCertificate());
...
//			"2.5.29.15", keyUsage.getDERObject());
	...
	BouncyCastleCertProcessingFactory factory2 = BouncyCastleCertProcessingFactory
			.getDefault();
	GlobusCredential vomsProxy = factory2.createCredential(globusCred.getCertificateChain(),
			globusCred.getPrivateKey(), globusCred.getStrength(),
			(int) globusCred.getTimeLeft(), GSIConstants.DELEGATION_FULL,
			globusExtensionSet);		
	
	GSSCredential gssCred = null;
	try {
		//gssCred = new GlobusGSSCredentialImpl(vomsProxyCredential.getVomsProxy(),
	...
	//
	//ManagePropertyFile.addToList("MYPROXY_USERNAMES", myProxyUsername);
