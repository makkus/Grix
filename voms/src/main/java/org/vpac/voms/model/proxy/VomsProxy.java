...
public class VomsProxy extends GridProxy {
	...
	public VomsProxy(File proxyFile) throws NoVomsProxyException {
		...
		if (proxyFile.exists())
			ac = extractAC();
		if (ac == null) {
			throw new NoVomsProxyException(
					"Could not parse proxy. Most likely this is not a voms proxy.");
		}
		vomsac = new VOMSAttributeCertificate(ac);
	}

	/**
	 * Creates an empty proxy. The creation is implemented in the createProxy
	 * method which is called with the init() method of the superclass.
	 * 
	 * @param proxyFile
	 * @param vo
	 * @param command
	...
	protected GlobusCredential createProxy(char[] passphrase,
		...
		vomsProxyCredential = new VomsProxyCredential(globusProxy, vo, command,
	...
	public static void main(String args[]) {
		...
		// GridProxy proxy = new VomsProxy(LocalProxy.getProxyFile(),
		// VO.getDefaultVO(), "b/Markus/G2/G2S1:TestRole");
		// GridProxy proxy = new VomsProxy(LocalProxy.getProxyFile(),
		// VO.getDefaultVO(), "B/Chris/G2/G2S1:TestRole");
		// GridProxy proxy = new VomsProxy(LocalProxy.getProxyFile(),
		// VO.getDefaultVO(), "A", null);
