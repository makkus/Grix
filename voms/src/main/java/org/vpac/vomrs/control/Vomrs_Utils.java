...
public class Vomrs_Utils {
	...
	public static ArrayList<String> getGroups(ArrayList<String> groups_long) {
		...
		for (String group_long : groups_long) {
			String group_short = group_long.substring(group_long.indexOf("/"),
					group_long.indexOf("/Role="));
	...
	/**
	 * Filters out the groups of the answer of the MbrInfo query to the VOMRS
	 * web service.
	 * 
	 * @param mbrInfoResult
	 *            the result of a MbrInfo query to a VOMRS web service
	...
	public static String[] getGroupRoles(String[] mbrInfoResult) {
		...
		String group_string = mbrInfoResult[14].substring(1,
				mbrInfoResult[14].length() - 1);

		String[] groups = group_string.split("\\]\\[");

		ArrayList<String> activeGroups = new ArrayList<String>();
		for (String group : groups) {
			if (group.indexOf(ROLE) != -1 && group.indexOf(STATUS) != -1
					&& group.indexOf(TYPE) != -1) {
