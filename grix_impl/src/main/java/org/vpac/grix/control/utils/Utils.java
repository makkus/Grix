...
public class Utils {
	...
	public static File backupFile(File file){
		...
		File bakFile = new File(file.toString()+"."+GrixProperty.getString("backup.file.extension"));
		int i = 1;
		while ( bakFile.exists() ) {
			bakFile = new File(file.toString()+"."+GrixProperty.getString("backup.file.extension")+i);
			i = i+1;
		}
...
//	}
	...
	/**
	 * Returns the name of the organization unit(order in config.properties)
	 * to which the search string fits.
	 * 
	 * @param domain
	 *            the search string
...
//	}
	...
	/**
	 * Returns the index of the organization unit(order in config.properties)
	 * to which the search string fits.
	 * 
	 * @param domain
	 *            the search string
		...
		for ( int i = 0; i < org_units.length; i++ ) {
			...
			if ( !domains[0].equals( "" ) ) {
				for ( int j = 0; j < domains.length; j++ ) {
					if ( domain.endsWith( domains[j] ) ) return org_units[i];
				}
			}
	...
	/**
	 * Checks whether the passphrase is ok with the passphrase policy of the APACGrid
	 * @param passphrase1 the passphrase	
	 * @return true if valid, false if not
	 */
	public static boolean isValidPassphrase(char[] passphrase1) {
		if ( passphrase1.length < GrixProperty.getInt( "minimal.passphrase.lenght" ) ) return false;
		else return true;
	}
	
	/** 
	 * Helper method, tests whether the two provided char-arrays are equal
	 * @param first first passphrase
	 * @param second second passphrase
	...
	public static boolean samePasswords(char[] first, char[] second) {
		...
		if ( first.length != second.length ) equal =  false;
