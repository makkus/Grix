...
public class CertificateStatusPanel extends JPanel implements
	...
	public void statusChanged(CertificateEvent event) {
		...
		if (CertificatePanel.CERT_EXPIRED.equals(status)) {
			...
			setMessage(Grix.getMessages().getString(status)
					...
					+ GlobusLocations.defaultLocations().getUserCertPKCS12()
							.toString() + "</a><p>"
					+ Grix.getMessages().getString(status + ".firefoxHelp")
					+ "</p>");
			...
			timer.stop();
			setCurrentIcon(redLight);
		} else if (CertificatePanel.RENEW_REQUESTED.equals(status)
				|| CertificatePanel.RENEW_REQUESTED.equals(status)) {
			timer.start();
			setCurrentIcon(yellowLight);
