...
public class MyProxyCred {
	...
	static public final String myProxyHost = GrixProperty.getString("myproxy.server"); 
	static public final int myProxyPort = GrixProperty.getInt("myproxy.port");
	static public final int myProxyKeylength = GrixProperty.getInt("myproxy.keysize");
	...
	public GlobusCredential createGlobusCredential(char[] key_passphrase, int lifetime) throws IOException, GeneralSecurityException{
		...
		globusCred = factory.createCredential(new X509Certificate[] { cert },
				key.getPrivateKey(), myProxyKeylength,
	...
	/**
	 * Uploads Proxy credentials to a MyProxy server. If there are already credentials with
	 * the same username on the server (created with the same certificate, of course) then
	 ...
	 * @throws GeneralSecurityException if you provide the wrong private key myProxyPassphrase
	 * @throws MyProxyException if the credential could not be created. (maybe the myProxyPassphrase for the MyProxy credential is too short or not complex enough, or the username already exists for another certificate, or...
	 * @throws IOException if the certificate / private key are not readable
	 * @throws GSSException I have not figured that out yet.
	 */
	public void init(GlobusCredential globusCred, boolean anonymous, char[] myProxyPassphrase, int credentials_lifetime_in_hours)
			throws GeneralSecurityException, MyProxyException, IOException, GSSException {

		if (myProxyPassphrase.length < GrixProperty
				.getInt("myproxy.minimal.password.length"))
			throw new MyProxyException("Passphrase must be at least 6 characters long.");

		GSSCredential gssCred = null;

		gssCred = new GlobusGSSCredentialImpl(globusCred,GSSCredential.INITIATE_AND_ACCEPT);
...
//		getInfo();
	 ...
	 * 
	 * @throws GSSException
	 *             if you don't have a credential file (/tmp/x509up_u...) to authenticate against
	 *             the myproxy server on your computer
