...
/**
 * This class is able to use a GenericClient to retrieve default / preselection values.
 * That only is useful if you know the name of the method you want to call and the type/amount
 * of the arguments that are necessary to call the method upfront.<p>
 * 
 * Very useful e.g. if you want to present the user with a selection of available
 * institions in a VO. This always uses the client the query is connected to.
 * 
 ...
 *</type>}
 * </pre>
 * <p>The <b>object</b> tag specifies the query to call. If one/several <b>client_args</b> tag(s) are
 * present, the query is called with these arguments.
 * 
...
public class CallGenericClient extends QcRetriever {
	...
	public CallGenericClient (QueryArgument arg) throws ArgumentsException{
		...
		for (int i = 0; i < arguments.size(); i++) {
			...
			throw new ArgumentsException("Could not init CallGenericClient retriever: "+e.getMessage());
		}
