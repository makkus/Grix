...
//	+ File.separator + ".globus" + File.separator + "grix.certificates";
	...
	/**
	 * This initializes the VOMRS web service connection by setting
	 * some environment variables. Do it before you connect to 
	 * a VOMRS web service.
	 * @throws CertificateException if the grid proxy is expired or not present
	 */
	public static void initVomrsWS() throws CertificateException {
		...
		if (LocalProxy.getStatus() != GridProxy.INITIALIZED) {
			myLogger.error("Could not find valid proxy, Proxy status: "+LocalProxy.getStatus());
			throw new CertificateException("No credentials found.");
		}
		System.setProperty("gridProxyFile", LocalProxy.getProxyFile().toString());
...
//		              				+ File.separator + "*.0";
		...
		if ( sslCAFiles == null )
			sslCAFiles = System.getProperty("user.home")+File.separator+".globus"+File.separator+"certificates";
		myLogger.debug("Setting \"sslCAFiles\" system property to: "+sslCAFiles);
		System.setProperty("sslCAFiles", sslCAFiles);
		//System.setProperty("sslCAFilese", CERTIFICATE_DIRECTORY + File.separator + "*.0");
		myLogger.debug("cacert_dir: " + System.getProperty("sslCAFiles"));
//		System.setProperty("crlFiles", GlobusLocations.defaultLocations()
//				.getCertificatesDirectories()[0]
//				+ "*.r0");
		//System.setProperty("crlFiles", CERTIFICATE_DIRECTORY + File.separator + "*.r0");
